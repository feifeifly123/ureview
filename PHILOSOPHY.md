...
- **站点代码不随数据更新 rebuild** (Iron rule #1)。成熟站 = "ship data, not code": 加一条 review 的完整流程是 `pnpm refresh:data:prod` (上 R2), **不 touch git, 不 push, 不触发 Cloudflare Pages rebuild**。任何需要"每新加一条 review 都 git push"的设计都是铁律违反。唯一例外: 真的改站点代码本身 (CSS / 组件 / schema 字段) 时才 push。
- **公共站是纯静态部署** (Iron rule #2)。`apps/web/` 的 build 产物 100% 是静态资源 (HTML / CSS / JS / 字体), 请求期我们自己的基础设施**不执行任何代码**。排除 Cloudflare Pages Functions / Workers / 边缘中间件 / SSR / on-demand rendering。唯一的动态计算在两处: CDN (缓存+TLS) 和读者浏览器 (client-side render)。数据层 `data.openagent.review` 也是静态: 从 R2 直出预生成的 JSON, 不是查询引擎。接受的代价: `/review/?id=X` 的分享预览是通用 title, 因为爬虫不跑 JS 而我们不在 edge 补 title。想改这条规则需要**在 README 写清楚为什么 + 新的 regime**, 不是悄悄加一个 Function 文件。详见 `README.md` 的两条 Iron rule 章节。
- **所有外网 tool 必须 SOCKS-safe** (Iron rule #3)。`tools/` 里任何会发 outbound HTTP 的脚本 (`fetch_hf.py`, `fetch_arxiv.py`, 未来的 `fetch_openreview.py` / `fetch_doi.py` 等) **必须**通过 `tools/_netlib.py` 的 `fetch()` / `fetch_text()` 发请求, 不得直接调 `urllib.request.urlopen`。理由: Python urllib 只读 HTTP_PROXY/HTTPS_PROXY 且**不懂 SOCKS**, 容器 egress 经常是 `socks5h://` 形式 — urllib 会用"Remote end closed connection"的姿势扑街。`_netlib.fetch` 统一做 "urllib → curl fallback", curl 懂 SOCKS 也懂 HTTP, 两种环境都能跑。**历史教训**: `fetch_arxiv.py` 原版没有 fallback, pipeline 从 trending pivot 到 daily 当天, studio 一点 "Open →" 就 502。不要再踩这个坑; 新 tool code review 时第一眼问"它是不是调了 `_netlib.fetch`?"。
...
- **Browse 页 review-aware filters**: ethics flagged, soundness concern, 按 ratings 维度筛。数据已经在 latest.json 里 flatten 好了, 只差 UI。
